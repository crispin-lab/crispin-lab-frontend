...
const server = createServer(async (req, res) => {
  // 핸들러 안에서 throw 가 발생하면 응답 없이 hang → spec 이 Playwright timeout 으로만 surface 해
  // 원인 추적이 어려워진다. 모든 분기를 try/catch 로 감싸 500 명시 응답으로 즉시 전환.
  try {
    ...
    const body =
      status >= 400 && handler.body === undefined
        ? {
            code: "MOCK_BODY_MISSING",
            message: `error body 누락 — ${key} 의 mock 에 body 명시 필요`,
          }
        : handler.body;
    sendJson(res, status, body);
  } catch (error) {
    if (!res.headersSent) {
      sendJson(res, 500, { code: "MOCK_INTERNAL", message: String(error) });
    } else {
      res.end();
    }
  }
